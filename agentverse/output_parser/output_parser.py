...
@output_parser_registry.register("misinformation_spread")
class MisinformationSpreadParser(OutputParser):
    def parse(self, output: LLMResult) -> Union[AgentAction, AgentFinish]:
        text = output.content
        action_result = re.findall(r"Action:(.+)", text)
        result = re.findall(r"Action:(.+?)Action Input:(.+)", text, re.DOTALL)
        ...
        action = action_result[0].strip()
        if action not in ["Listen"]:
            if len(result) == 0:
                raise OutputParserError(text)
            action_input = result[0][1].strip()

        if action == "Speak":
            return AgentFinish({"output": "[Speak] " + action_input}, text)
        elif action == "CallOn":
            return AgentFinish({"output": "[CallOn] " + action_input}, text)
        elif action == "Listen":
            return AgentFinish({"output": "[Listen]"}, text)
        elif action == "Query":
            return AgentFinish({"output": "[Query] Team_B, do you believe that HIV is a biochemical weapon manufactured in the United States?"}, text)
        elif action == "Answer":
            return AgentFinish({"output": "[Answer] " + action_input}, text)
        elif action == "GroupDiscussion": # professor ask the group to discuss about the topic
            return AgentFinish({"output": "[GroupDiscussion] " + action_input}, text)
        elif action == "GroupDiscuss": # Team_B discuss about the topic
            return AgentFinish({"output": "[GroupDiscuss] " + action_input}, text)
        else:
            return AgentAction(action, action_input, text)
        
@output_parser_registry.register("misinformation_6players")
class Misinformation6PlayersSpreadParser(OutputParser):
    def parse(self, output: LLMResult) -> Union[AgentAction, AgentFinish]:
        text = output.content
        action_result = re.findall(r"Action:(.+)", text)
        result = re.findall(r"Action:(.+?)Action Input:(.+)", text, re.DOTALL)
        ...
        action = action_result[0].strip()
        if action not in ["Listen", "RaiseHand", "MoveToFront", "MoveBack"]:
            if len(result) == 0:
                raise OutputParserError(text)
            action_input = result[0][1].strip()
        else:
        ...
        if action == "Listen":
            return AgentFinish({"output": "(Silence)"}, text)
        if action == "RaiseHand":
            return AgentFinish({"output": "[RaiseHand]"}, text)
        if action == "MoveToFront":
            return AgentFinish({"output": "[MoveToFront]"}, text)
        if action == "MoveBack":
            return AgentFinish({"output": "[MoveBack]"}, text)
        if action == "Present":
            return AgentFinish({"output": "[Present] " + action_input}, text)
        if action == "Speak":
            return AgentFinish({"output": "[Speak] " + action_input}, text)
        if action == "Respond":
            return AgentFinish({"output": "[Respond]"  + action_input}, text)
        if action == "CallOn":
            return AgentFinish({"output": "[CallOn] " + action_input}, text)
        if action == "Query":
            return AgentFinish({"output": "[Query]: Do you believe the presentation?"}, text)
        else:
            return AgentAction(action, action_input, text)

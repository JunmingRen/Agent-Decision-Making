...
@SelectorRegistry.register("misinformation_6players")
class Misinformation6PlayersSelector(BaseSelector):
    def select_message(
        ...
        for message in messages:
            ...
            if message.sender.startswith("Student"):
                if environment.agents[0].current_action == "Speak":
                    if message.content.startswith("[RaiseHand]"):
                        selected.append(message)
                        environment.update_action_by_agent(current_agent, "RaiseHand")
                    else:
                        print(f"Error: {message.sender} sent {message.content} while in Speak state. Expected [RaiseHand]")
                        message.content = "[Listen]"
                        selected.append(message)
                        environment.update_action_by_agent(current_agent, "Listen")

                elif environment.is_called_on(current_agent) and message.content.startswith("[MoveToFront]"):
                    selected.append(message)
                    environment.update_action_by_agent(current_agent, "MoveToFront")
                elif current_action == "MoveToFront" and message.content.startswith("[Present]"):
                    selected.append(message)
                ...
                if current_action == "" and message.content.startswith("[Speak]"):
                    selected.append(message)
                    environment.update_action_by_agent(current_agent, "Speak")
                elif current_action == "Speak"  and message.content.startswith("[CallOn]"):
                    selected.append(message)
                ...
                elif current_action == "Query" and message.content.startswith("[Speak]"):
                    selected.append(message)
                    environment.update_action_by_agent(current_agent, "Speak")

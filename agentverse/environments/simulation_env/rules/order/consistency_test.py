...
if TYPE_CHECKING:
    from agentverse.environments import BaseEnvironment, MisinformationEnvironment, ConsistencyTestEnvironment


@OrderRegistry.register("consistency_test")
class ConsistencyTestOrder(BaseOrder):
    ...
    def get_next_agent_idx(self, environment: ConsistencyTestEnvironment) -> List[int]:
            ...
            if sender.startswith("Professor"):
                if content.startswith("[CallOn]"):
                    ...
                    if result is not None:
                        ...
                        if self.current_presentor == (agent_cnt - 1):
                            environment.is_checking = True
                        return [self.current_presentor]
                elif content.startswith("[Query]"):
                    # 2. professor asks do they believe ...
                    print("Querying")
                    environment.agents[self.current_presentor].visited = True
                ...
                if content.startswith("[Present]"):
                    environment.update_presenting(self.current_presentor)
                if content.startswith("[MoveToFront]"):
                    return [self.current_presentor]
                else:
                    return [0]
        else:
